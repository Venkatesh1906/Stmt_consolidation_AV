...
for asset, col_to_take in zip(bos_desc['Code'], bos_desc['MV Col']):
 
    if not asset in bos_df['SubAssetType'].unique().tolist():
        print(f"Asset Not Found {asset}")
        
    if not col_to_take in bos_df.columns.tolist():
        print(f"MV Col Not found {col_to_take}")
    try:
        bos_df['MV_value'] = np.where(bos_df['SubAssetType'] == asset, \
            bos_df[col_to_take], \
                bos_df['MV_value'])
    except Exception as exc:
        print (f"Exception occured at {asset}, {col_to_take}")

bos_df.to_csv('./Output/bos_mv_col_v0.3.csv', index=False)